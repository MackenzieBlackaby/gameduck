    ...
    public void execute() {
        ...
        switch (type) {
            ...
            case SP_PLUS_IMMEDIATE8_HL:
                ...
                cpu.setFlag(Flag.H, (spValue & 0xF) + (offset & 0xF) > 0xF);
                cpu.setFlag(Flag.C, (spValue & 0xFF) + (offset & 0xFF) > 0xFF);
                break;
            // Additions
            ...
            case CP_REGISTER:
                byte cpFromReg = cpu.regGet(Register.A);
                byte cpWithReg = cpu.regGet(Register.getRegFrom3Bit(values[0]));
                short cpResultReg = (short) (cpFromReg - cpWithReg);
                cpu.alu.updateSubFlags(cpResultReg, cpFromReg, cpWithReg);
                break;
            case CP_MEMORY_HL:
                byte cpFromMem = cpu.regGet(Register.A);
                byte cpWithMem = memory.read(cpu.regGet16(Register.HL));
                short cpResultMem = (short) (cpFromMem - cpWithMem);
                cpu.alu.updateSubFlags(cpResultMem, cpFromMem, cpWithMem);
                break;
            case CP_IMMEDIATE:
                byte cpFromImmediate = cpu.regGet(Register.A);
                byte cpWithImmediate = values[0];
                short cpResultImmediate = (short) (cpFromImmediate - cpWithImmediate);
                cpu.alu.updateSubFlags(cpResultImmediate, cpFromImmediate, cpWithImmediate);
                break;
            case INC_REGISTER:
                Register incReg = Register.getRegFrom3Bit(values[0]);
                cpu.regSet(incReg, cpu.idu.calculate(cpu.regGet(incReg), true));
                break;
            case INC_MEMORY_HL:
                short incAddr = cpu.regGet16(Register.HL);
                memory.write(incAddr, cpu.idu.calculate(memory.read(incAddr), true));
                break;
            case DEC_REGISTER:
                Register decReg = Register.getRegFrom3Bit(values[0]);
                cpu.regSet(decReg, cpu.idu.calculate(cpu.regGet(decReg), true));
                break;
            case DEC_MEMORY_HL:
                short decAddr = cpu.regGet16(Register.HL);
                memory.write(decAddr, cpu.idu.calculate(memory.read(decAddr), true));
                break;
            default:
